...
```sh
make docker-build docker-push IMG=<some-registry>/tb-deployment-webhook:tag
```
...
```sh
make deploy IMG=<some-registry>/tb-deployment-webhook:tag
```
...
```sh
make build-installer IMG=<some-registry>/tb-deployment-webhook:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/tb-deployment-webhook/<tag or branch>/dist/install.yaml
```
